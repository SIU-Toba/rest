...
class autenticacion_ssl extends proveedor_autenticacion
{
    ...
    public function get_usuario(request $request = null)
    {
        ...
        if (!$hay_serial || !$hay_vto || !$cert_valido || !$hay_CA) {
            ...
            if (!$cert_valido) {
                ...
                $mensaje = "No se defini贸 una CA en el Servidor Web (en apache la CA es la variable SSLCACertificateFile)";
            } else if (! $hay_vto) {
        ...
        if ($_SERVER['SSL_CLIENT_V_REMAIN'] <= 0) {                     //Le quedan dias de validez al cert?
            $this->set_error("El certificado de cliente est谩 expirado");
            return;
        }
        ...
        if (isset($_SERVER['SSL_CLIENT_S_DN_CN']) && trim($_SERVER['SSL_CLIENT_S_DN_CN']) != '') {   //Busco el nombre del cliente
            ...
            if ($this->es_valido($user, $cert)) {
                ...
                $this->set_error("El certificado de cliente es v谩lido, pero no se encuentra en el repositorio de usuarios");
            }
        } else {
    ...
    {
        $rta = $rta->set_data(array('mensaje' => \utf8_e_seguro('autenticaci髇 cancelada, falta informaci髇')));
    }
